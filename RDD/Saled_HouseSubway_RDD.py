...
# 清洗不存在的数据
pairRDD1 = pairRDD.filter(lambda row:row[33] != '不存在该项' and row[33] != -1)
# 初步提取数据，汇集成元组
pairRDD2 = pairRDD1.map(lambda row:(row[33], (row[7] , row[6])))
# 对数据进行聚合
...
# 获得每一个数据项的值
ID = []; Saled_HouseHasSubway = []; Saled_HouseHasSubway_Number = []
idcount = 0
for index,element in result:
    ID.append(idcount)
    Saled_HouseHasSubway.append(element[0])
    Saled_HouseHasSubway_Number.append(element[1])
    idcount = idcount + 1
print(Saled_HouseHasSubway)
print(Saled_HouseHasSubway_Number)
# 将获得的值存入数据库中
for i in range(0,idcount):
    sql = "INSERT INTO Saled_HasSubway VALUES ({},'{}',{})"\
        .format(ID[i],Saled_HouseHasSubway[i],Saled_HouseHasSubway_Number[i])
    try:
