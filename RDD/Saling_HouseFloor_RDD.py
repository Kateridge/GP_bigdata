...
spark =SparkSession.builder.appName("Saling_Floor").master("local[*]").getOrCreate()
...
# 清洗不存在的数据
pairRDD1 = pairRDD.filter(lambda row:row[10] != '不存在该项')
# 初步提取数据，汇集成元组
pairRDD2 = pairRDD1.map(lambda row:(row[10].split("(")[0], (row[5] , row[4])))
# 对数据进行聚合
...
# 获得每一个数据项的值
ID = []; Saling_HouseFloor = []; Saling_HouseFloor_Number = []
idcount = 0
for index,element in result:
    ID.append(idcount)
    Saling_HouseFloor.append(element[0])
    Saling_HouseFloor_Number.append(element[1])
    idcount = idcount + 1
print(Saling_HouseFloor)
print(Saling_HouseFloor_Number)
# 将获得的值存入数据库中
for i in range(0,idcount):
    sql = "INSERT INTO Saling_HouseFloor VALUES ({},'{}',{})"\
        .format(ID[i],Saling_HouseFloor[i],Saling_HouseFloor_Number[i])
    try:
