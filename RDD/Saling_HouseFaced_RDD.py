...
spark =SparkSession.builder.appName("Saling_HouseFaced").master("local[*]").getOrCreate()
...
# 清洗不存在的数据
pairRDD1 = pairRDD.filter(lambda row:row[15] != '不存在该项' and row[15] != '毛坯' and row[15] != '精装' and row[15] != '简装')
# 初步提取数据，汇集成元组
pairRDD2 = pairRDD1.map(lambda row:(row[15].split(" ")[0], (row[5] , row[4])))
# 对数据进行聚合
...
# 获得每一个数据项的值
ID = []; Saling_HouseFaced = []; Saling_HouseFaced_Number = []
idcount = 0
for index,element in result:
    ID.append(idcount)
    Saling_HouseFaced.append(element[0])
    Saling_HouseFaced_Number.append(element[1])
    idcount = idcount + 1
print(Saling_HouseFaced)
print(Saling_HouseFaced_Number)
# 将获得的值存入数据库中
for i in range(0,idcount):
    sql = "INSERT INTO Saling_HouseFaced VALUES ({},'{}',{})"\
        .format(ID[i],Saling_HouseFaced[i],Saling_HouseFaced_Number[i])
    try:
