...
# 初步提取数据，汇集成元组
pairRDD2 = pairRDD1.map(lambda row:(row[1].split("_")[1], (row[7] , row[6])))
# 对数据进行聚合
...
for index,element in result:
    ID.append(idcount)
    Saled_District.append(info.area_info[element[0]])
...
for i in range(0,idcount):
    sql = "INSERT INTO Saled_Sort_District_By_Number VALUES ({},'{}',{},{},{})"\
