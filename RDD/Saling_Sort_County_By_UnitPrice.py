...
spark =SparkSession.builder.appName("Sort_County").master("local[*]").getOrCreate()
...
# 清洗不存在的数据
pairRDD1 = pairRDD.filter(lambda row:row[7] != '不存在该项')
# 初步提取数据，汇集成元组
pairRDD2 = pairRDD1.map(lambda row:(row[7], (row[5] , row[4])))
# 对数据进行聚合
...
# 获得每一个数据项的值
ID = []; Saling_County = []; Saling_County_Number = []; Saling_UnitPrice = []; Saling_TotalPrice = [];
idcount = 0
for index,element in result:
    ID.append(idcount)
    Saling_County.append(element[0])
    Saling_County_Number.append(element[1])
...
for i in range(0,idcount):
    sql = "INSERT INTO Saling_Sort_County_By_UnitPrice VALUES ({},'{}',{},{},{})"\
        .format(ID[i],Saling_County[i],Saling_County_Number[i],Saling_UnitPrice[i],Saling_TotalPrice[i])
    try:
