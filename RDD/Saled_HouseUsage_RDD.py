...
# 清洗不存在的数据
pairRDD1 = pairRDD.filter(lambda row:row[29] != '不存在该项')
# 初步提取数据，汇集成元组
pairRDD2 = pairRDD1.map(lambda row:(row[29].split(" ")[0], (row[7] , row[6])))
# 对数据进行聚合
...
# 获得每一个数据项的值
ID = []; Saled_HouseUsage = []; Saled_HouseUsage_Number = []
idcount = 0
for index,element in result:
    ID.append(idcount)
    Saled_HouseUsage.append(element[0])
    Saled_HouseUsage_Number.append(element[1])
    idcount = idcount + 1
print(Saled_HouseUsage)
print(Saled_HouseUsage_Number)
# 将获得的值存入数据库中
for i in range(0,idcount):
    sql = "INSERT INTO Saled_HouseUsage VALUES ({},'{}',{})"\
        .format(ID[i],Saled_HouseUsage[i],Saled_HouseUsage_Number[i])
    try:
