...
spark =SparkSession.builder.appName("Saling_HouseHasSubway").master("local[*]").getOrCreate()
...
# 清洗不存在的数据
pairRDD1 = pairRDD.filter(lambda row:row[28] != '不存在该项' and row[28] != -1)
# 初步提取数据，汇集成元组
pairRDD2 = pairRDD1.map(lambda row:(row[28], (row[5] , row[4])))
# 对数据进行聚合
...
# 获得每一个数据项的值
ID = []; Saling_HouseHasSubway = []; Saling_HouseHasSubway_Number = []
idcount = 0
for index,element in result:
    ID.append(idcount)
    Saling_HouseHasSubway.append(element[0])
    Saling_HouseHasSubway_Number.append(element[1])
    idcount = idcount + 1
print(Saling_HouseHasSubway)
print(Saling_HouseHasSubway_Number)
# 将获得的值存入数据库中
for i in range(0,idcount):
    sql = "INSERT INTO Saling_HasSubway VALUES ({},'{}',{})"\
        .format(ID[i],Saling_HouseHasSubway[i],Saling_HouseHasSubway_Number[i])
    try:
